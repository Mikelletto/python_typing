...
def fetch_breweries(limit: int = 20, city: str | None = None):
    url = f"https://api.openbrewerydb.org/v1/breweries?per_page={limit}"
    if city:
        # Zakładamy city np. "San Diego" — zakoduj, np. podmieniając spację:
        city_param = city.replace(" ", "_")
        url += f"&by_city={city_param}"
    response = requests.get(url)
    response.raise_for_status()
    return response.json()

# Konfiguracja argparse
parser = argparse.ArgumentParser(description="Pobiera listę browarów z API OpenBreweryDB")
parser.add_argument("--city", type=str, help="Ograniczenie wyników do miasta")
args = parser.parse_args()

# Pobranie danych z API
try:
    dane = fetch_breweries(limit=20, city=args.city)
